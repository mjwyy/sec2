...
public class TransitNoteInputData extends NoteInputData implements TransitNoteInputDataService {
    ...
    public ResultMsg insert(TransitNotePO po) throws RemoteException, SQLException, ElementNotFoundException {
        ...
        statement.setString(1,po.getTransitDocNumber());
        statement.executeUpdate();
        statement.close();

        //记录系统日志
        logInsertData = new LogInsertData();
        logInsertData.insertSystemLog("营业厅业务员?新增营业厅到达单,单据编号:" + po.getTransitDocNumber());

        //等待总经理审批过程,反复查询
        DocState result = this.waitForCheck("note_arrival_on_transit",
                "transferNumber", po.getTransitDocNumber());
